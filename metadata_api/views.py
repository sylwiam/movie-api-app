...
def migrateRecords(request):
	...
	for record in records:
		# convert genre from a string into a list, so that we can loop through it
		...
		try:
			for genre in genreList:
				...
				# save new genre record into genre table to get object if exists
				genreObject, createdGenre = Genre.objects.get_or_create(name=genre)

				if createdGenre:
					newGenreCouner += 1

			# save new movie record into movie table to get object if exists
			...
			# save new moviegenre record into moviegenre table to get object if exists
			movieGenreObject, createdMovieGenre = MovieGenre.objects.get_or_create(
				movie_pk=movieObject,
				genre_pk=genreObject)
			...
			# extract error type, error object, line number, file name and traceback to generate detailed error message
